...
class Env(VecEnvWrapper):
	def __init__(self, num_processes):
		self.envs = [gym.make('Breakout-v0') for i in range(num_processes)]
	...
	def action():
		return Env.envs.action_space.sample()
...
class Agent():
	...
	def step():
		action = Env.action()
		Env.envs.step_async(action)
		obs, reward, done, info = Env.envs.step_wait()
...
class replay_buffer():
	...
	def add(self):
		state, action, reward, done = Agent.step()
...
replay_buffer(32, 32, 32, 32).sample()
# class ppo
